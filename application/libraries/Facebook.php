...
Class Facebook
{
    ...
    public function login_url_user(){
        // Login type must be web, else return empty string
        if($this->config->item('facebook_login_type') != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . $this->config->item('facebook_login_user_redirect_url'),
            $this->config->item('facebook_permissions')
        );
    }
    ...
    public function videoToUpload($path){
        return $this->fb->videoToUpload($path);
    }

    /*public function delete(){
        return $this->fb->delete('me');
    }*/
}